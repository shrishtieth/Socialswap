...
contract Staking is ReentrancyGuard, Pausable, RewardsDistributionRecipient, IStakingRewards {
    using _SafeMath for uint256;
    ...
    function earned(address account) public view override returns (uint256) {
        return _balance_SST[account].mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    ...
    // withdraw funds
     ...
     function transferSST(address _wallet, uint256 amount) external onlyOwner {
         require( amount <= SST_token.balanceOf(address(this)));
         SST_token.safeTransfer(_wallet, amount);
    }
    ...
    function getReward() public override nonReentrant updateReward(msg.sender) notPaused {
        uint256 reward = rewards[msg.sender];
        if (reward > 0) {
        rewards[msg.sender] = 0;
        SST_token.safeTransfer(msg.sender, reward);
    ...
    function notifyRewardAmount(uint256 reward) external override onlyRewardsDistribution updateReward(address(0)) {
        ...
        uint balance = SST_token.balanceOf(address(this)).sub(_totalSupply);
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        ...
        require(
            tokenAddress != address(SST_token),
            "Cannot withdraw the staking or rewards tokens"
        );
        ITRC20(tokenAddress).safeTransfer(owner, tokenAmount);
